...
/* IN THIS PROJECT Serial carries the raw I/Q stream - text goes to Serial0. */
#ifndef RGB_OUT
#define RGB_OUT Serial0
#endif

/* arduino-esp32 3.x: both rgbLedWrite and neopixelWrite exist */
#ifndef RGB_WRITE
...
typedef struct {
    int      pin;
    uint8_t  fenyero;      /* peak brightness 1..255                      */
    float    lassu_s;      /* cycle period on an idle system              */
    float    gyors_s;      /* cycle period at 100% load                   */
    float    gorbe;        /* 0.1..2.0 exponent; 0.5 = square root        */
    uint8_t  dither;
    uint8_t  gamma_on;
...
/* Explicit read-modify-write: volatile ++ is deprecated in C++20
 * (-Wvolatile), because it is not atomic and its semantics are ambiguous.
 * Atomicity is not needed here anyway: per core there is ONE writer (that
 * core's idle task) and one reader, and a lost count within a 500 ms
 * window is immeasurable. */
...
/* At the end of a window: load = 1 - counter/max.  The max self-calibrates:
 * it follows upward immediately, decays slowly downward (otherwise a single
 * loaded window would depress the reference forever). */
static void load_update(float dt_s)
{
    for (int c = 0; c < 2; c++) {
        ...
        /* normalise to 1 second, so the window length does not matter */
        uint32_t per_s = (dt_s > 0.0f) ? (uint32_t)(n / dt_s) : n;

        if (per_s > s_idle_max[c]) s_idle_max[c] = per_s;              /* up at once  */
        else s_idle_max[c] = (uint32_t)(s_idle_max[c] * 0.999f + 1);   /* slowly down */

        float l = 1.0f - (float)per_s / (float)(s_idle_max[c] ? s_idle_max[c] : 1);
        if (l < 0.0f) l = 0.0f;
        if (l > 1.0f) l = 1.0f;

        /* smoothing, so the LED does not jitter */
...
static volatile int  s_test = 0;      /* >0: colour-wheel test, ms remaining */

static void rgb_task(void *arg)
{
    (void)arg;
    float hue = 0.0f;
    float err_r = 0, err_g = 0, err_b = 0;     /* temporal dither error terms */
    ...
    while (s_run) {
        ...
        /* load measurement every half second */
        ...
        /* --- cycle period from the load ---
         * The curve (exponent) is needed because linearly a 10-20% load
         * would barely show; with a square-root mapping it is noticeable:
         *   20% -> 50 s (sqrt)  vs  72 s (linear), from a 90 s base. */
        ...
        /* --- colour ---
         * First compute in the 0..1 range, apply gamma, and ONLY then scale
         * to the brightness. (The other way round, dim colours would vanish.) */
        float r, g, b;
        ...
        uint8_t R, G, B;
        if (s_cfg.dither) {
            /* Temporal dither: the fractional part is carried into the next
             * frame. At 12/255 brightness the green channel of hue=10 is
             * 0.23 - rounded it would be LOST, and the first 15 degrees of
             * the transition would stay pure red. At 50 Hz the eye
             * averages, and even the 0.23 is visible. */
...
void rgb_load_init(int pin)
{
    ...
    s_run = true;
    /* core 0, priority 1: beside the display task, below the FG23 SPI */
    if (xTaskCreatePinnedToCore(rgb_task, "rgb_load", 3072, NULL, 1, &s_task, 0) != pdPASS) {
        s_run = false;
        RGB_OUT.println("RGB: task start failed");
        return;
    }
    RGB_OUT.printf("RGB: load indicator running on pin %d (idle hook c0=%d c1=%d)\n",
...
static void rgb_print_cfg(void)
{
    RGB_OUT.printf("RGB-CFG: pin=%d fenyero=%u lassu=%.1fs gyors=%.1fs gorbe=%.2f "
                   "dither=%u gamma=%u telitettseg=%.2f hz=%u run=%d | "
                   "load c0=%.0f%% c1=%.0f%% (%s)\n",
...
static void rgb_set_kv(const char *args)
{
    ...
    for (char *t = strtok_r(buf, " \t,", &save); t; t = strtok_r(NULL, " \t,", &save)) {
        ...
        *eq = 0;
        ...
        else RGB_OUT.printf("RGB: unknown key: %s\n", k);
    }
...
bool rgb_load_cmd(const char *line)
{
    ...
    if (!strncmp(p, "test", 4)) { s_test = 3000; RGB_OUT.println("RGB: colour-wheel test 3 s"); return true; }
    if (p[0] == '0') { rgb_load_stop(); RGB_OUT.println("RGB: off"); return true; }
